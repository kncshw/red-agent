...
<p>The red team agent has identified an <strong>unauthenticated service exposed on the public internet</strong>
...
<ol>
  ...
  <li>Engage asset owner to implement authentication</li>
...
<p style="color:#999; font-size:12px;">Generated by BIS Red Team Agent (Gemma4-26B-abliterated) | oh-red-agent</p>
    ...
    plain = (
        f"[RED TEAM ESCALATION] Unauthenticated {application} exposed — {ip}:{port}\n\n"
        f"Target      : {ip}:{port}\n"
        f"Application : {application}\n"
        f"Auth Status : NO AUTHENTICATION\n"
        f"Evidence    : {evidence}\n\n"
        ...
        f"4. Engage asset owner to implement authentication\n"
        f"5. Re-scan after remediation to confirm closure\n\n"
